...
import hxcore.macros.HaxeCBridge.CodeTools.*;
...
class HaxeCBridge {
		...
		function convertFunction(f: ClassField, kind: FunctionInfoKind) {
			...
			switch fieldExpr.expr {
				case TFunction(tfunc):
					...
					var cleanDoc = f.doc != null ? StringTools.trim(CodeTools.removeIndentation(f.doc)) : null;
	...
	static function generateHeader(ctx: CConverterContext, namespace: String) {
		...
		')
		+ CodeTools.indent(1, ctx.supportFunctionDeclarations.map(fn -> CPrinter.printDeclaration(fn, true, prefix)).join(';\n\n') + ';\n\n')
		+ CodeTools.indent(1, ctx.functionDeclarations.map(fn -> CPrinter.printDeclaration(fn, true, prefix)).join(';\n\n') + ';\n\n')
	...
	static function generateImplementation(ctx: CConverterContext, namespace: String) {
		...
		')
		+ (if (isDynamicLink()) CodeTools.code('
		...
		if (externalThread) {
			...
			return (
				...
				+ CodeTools.indent(1,
					...
					')
					+ if (hasReturnValue) CodeTools.code('
