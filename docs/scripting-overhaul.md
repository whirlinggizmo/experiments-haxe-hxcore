...
When generating cppia, Haxe uses `export_classes.info` to avoid compiling classes that already exist in the host application. This prevents duplicate classes and symbol collisions. Note that `export_classes.info` is generated by **hxcpp** when `-D scriptable` is enabled. However, `export_classes.info` may include classes that must still be present in the cppia output (for example, a base `Script` class or other shared runtime classes). The overhaul must explicitly handle filtering or altering this list so that required classes are included in generated cppia, even if they exist in the host build.
...
- HCR determinism: tests should force watcher scans (e.g., `ScriptLoader.tickWatchers()`), set debounce to `0`, and drive `MainLoop.tick()` during polling to avoid reliance on real-time timers.
...
# Refactor Guardrails
- Perform regular checkpoints to confirm tests are still passing.
- Perform regular design checks to ensure we continue to adhere to strong engineering principles (separation of concerns, single-source-of-truth configuration, minimal branching, and testability).

# Checkpoint Cadence
- After each major refactor step, run `haxe hxml/tests.hxml`.
- After each major refactor step, do a quick architectural review against the proposal and guardrails.
...
- Cpp test build output defaults to `out/tests-cpp/` (including `obj/`).
