...
class Console {
	...
	static var unicodeCompatibilityMode:UnicodeCompatibilityMode = #if ((sys || nodejs) && !macro) Sys.systemName() == 'Windows' ? Windows : #end
	None;
	...
	macro static public function debug(rest:Array<Expr>) {
		if (!Context.getDefines().exists('debug'))
			return macro null;
		rest = rest.map(removeMarkupMeta);
		#if haxe4
		...
		var result = formatTagPattern.map(s, function(e) {
			...
			if (!open && tags.length == 1) {
				...
				for (tag in tags) {
					...
					if (flag == null)
						return e.matched(0); // unhandled tag, don't treat as formatting
					if (open) {
				...
				case AsciiTerminal:
					...
					if (open) {
						...
						return getAsciiFormat(FormatFlag.RESET)
							+ activeFormatFlagStack.map(function(f) return getAsciiFormat(f)).filter(function(s) return s != null).join('');
					}
				#if js
				case BrowserConsole:
					browserFormatArguments.push(activeFormatFlagStack.map(function(f) return getBrowserFormat(f))
	...
	**/
	...
	static function printFormatted(s:String = '', outputStream:ConsoleOutputStream = Log) {
		...
		if (formatMode == BrowserConsole) {
			...
			switch outputStream {
				case Log, Debug:
					js.Syntax.code('console.Log.apply(console, {0})', logArgs);
				case Warn:
					js.Syntax.code('console.warn.apply(console, {0})', logArgs);
				case Error:
					js.Syntax.code('console.error.apply(console, {0})', logArgs);
			}
			#elseif haxe3
			switch outputStream {
				case Log, Debug:
					untyped __js__('console.Log.apply(console, {0})', logArgs);
				case Warn:
					untyped __js__('console.warn.apply(console, {0})', logArgs);
				case Error:
					untyped __js__('console.error.apply(console, {0})', logArgs);
			}
			#end
			#end
			return;
	...
	static function print(s:String = '', outputStream:ConsoleOutputStream = Log) {
		...
		switch outputStream {
			...
			case Warn:
				js.Syntax.code('console.warn({0})', s);
			case Error:
				js.Syntax.code('console.error({0})', s);
		}
		#elseif haxe3
		switch outputStream {
			case Log, Debug:
				untyped __js__('console.log({0})', s);
			case Warn:
				untyped __js__('console.warn({0})', s);
			case Error:
				untyped __js__('console.error({0})', s);
		}
		#end
		#end
		#end // #if (!no_console)
	...
	static function rgbToAscii256(r:Int, g:Int, b:Int):Null<Int> {
		...
		var ir = nearIdx(r, colorSteps),
			ig = nearIdx(g, colorSteps),
			ib = nearIdx(b, colorSteps);
		var ier = Math.abs(r - colorSteps[ir]),
			ieg = Math.abs(g - colorSteps[ig]),
			ieb = Math.abs(b - colorSteps[ib]);
		var averageColorError = ier + ieg + ieb;

		// Gray scale values are 232 to 255 inclusive = 24 colors
		// Steps are in spaces of 10
		// 0x08 + 10 * n = c
		var jr = Math.round((r - 0x08) / 10),
			jg = Math.round((g - 0x08) / 10),
			jb = Math.round((b - 0x08) / 10);
